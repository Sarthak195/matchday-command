...
export default function Dashboard() {
  ...
  useEffect(() => {
    dispatch({ type: "reset" });
    const source = new EventSource(`/api/stream?tickMs=${tickMs}`);
  ...
  async function generateDoc(type: "briefing" | "handover") {
    ...
    try {
      ...
      setDoc(type === "briefing" ? { kind: "briefing", data } : { kind: "handover", data });
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#0d0d0d] text-[#c3c2b7]">
        ...
        <div className="mx-auto flex max-w-[1400px] flex-wrap items-center gap-x-6 gap-y-3 px-5 py-3">
          ...
          <div className="ml-auto flex flex-wrap items-center gap-2">
            <label className="flex items-center gap-1.5 text-xs text-[#898781]">
              ...
              >
                <option value={2000}>1× (restarts)</option>
                <option value={500}>4× (restarts)</option>
              </select>
            </label>
            <button
      ...
      <main className="mx-auto grid max-w-[1400px] gap-4 px-5 py-4 lg:grid-cols-[300px_minmax(0,1fr)_380px]">
        {/* Zones & gates */}
        ...
        {/* Event ticker */}
        <Panel title="Live feed" bodyClassName="max-h-[78vh] overflow-y-auto">
...
function Panel({
  ...
  return (
    <section className="rounded-lg border border-white/10 bg-[#1a1a19]">
