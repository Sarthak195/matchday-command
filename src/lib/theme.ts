...
/** Lighter red for critical *text* on dark surfaces — the #d03b3b background
 *  tone fails WCAG AA contrast when used as text. Backgrounds keep STATUS.critical. */
export const CRITICAL_TEXT = "#f2726e";
...
export const SEVERITY_META: Record<IncidentSeverity, { label: string; color: string }> = {
  ...
  critical: { label: "Critical", color: CRITICAL_TEXT },
};

export function densityState(pct: number): { label: string; color: string } {
  if (pct >= DENSITY_CRITICAL_PCT) return { label: "Critical", color: CRITICAL_TEXT };
